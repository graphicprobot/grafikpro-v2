...
def handle_manual_phone(chat_id, phone):
    ...
    STATES[str(chat_id)]["state"] = "manual_service"
    STATES[str(chat_id)]["client_phone"] = phone_clean
...
def handle_manual_service(chat_id, service_name):
    ...
    for i in range(14):
        ...
        label = "Сегодня" if i == 0 else (datetime.now() + timedelta(days=i)).strftime('%d.%m (%a)')
        buttons.append([{"text": label, "callback_data": f"man_date_{date}"}])
...
def handle_manual_time(chat_id, time):
    ...
    firestore_add("appointments", {
        "master_id": str(chat_id), "client_name": state.get("client_name", ""),
        "client_phone": state.get("client_phone", ""), "service": state.get("service", ""),
        "date": state.get("date", ""), "time": time, "status": "confirmed"
    })
...
def handle_services_settings(chat_id):
    ...
    services = [s for s in master.get("services", []) if isinstance(s, dict) and s.get("name")]
    send_message(chat_id, "💈 *Услуги:*" if services else "💈 Нет услуг.", reply_markup=services_list_keyboard(services))
...
def handle_add_break_prompt(chat_id):
    ...
    send_message(chat_id, "🚫 Введите время перерыва:\nНапример: `13:00-14:00`", reply_markup={"keyboard": [["🔙 Отмена"]], "resize_keyboard": True})
...
def handle_client_date_select(chat_id, link_id, date):
    ...
    for h in range(start_h, end_h + 1):
        slot = f"{h}:00"
        if slot in busy: buttons.append([{"text": f"❌ {slot} (занято)", "callback_data": "ignore"}])
        elif any(f"{h}:00" >= b.split("-")[0] and f"{h}:00" < b.split("-")[1] for b in breaks): buttons.append([{"text": f"🚫 {slot} (перерыв)", "callback_data": "ignore"}])
